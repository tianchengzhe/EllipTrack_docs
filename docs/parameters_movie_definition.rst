...
*  :reditalic:`filename_format` :gray:`(string)` -- Formats of image filenames.
   ...
   .. admonition:: Example
      ...
      **Example 1 (Image Sequences)**. 
      Filenames follow the convention `RowID_ColumnID_SiteID_ChannelName_FrameID.tif`. Frame ID has a minimal length of three digits, filled with prefix 0. For example, `1_2_3_CFP_004.tif`.
      ...
      **Example 2 (Nikon ND2 Format)**. 
      Filenames follow the convention `WellRowIDColumnID_CFP,YFP,mCherry_Seq00XX.nd2`. Row ID is an upper-case letter; Column ID is a two-digit number, filled with prefix 0; and *XX* represents an arbitrary number. For example, `WellA02_CFP,YFP,mCherry_Seq0001.nd2`.
