...
Note: This tutorial was written with an earlier version of EllipTrack. The scripts for Segmentation have been updated since. There is no change in execution, but do expect very minor difference in the tracking results. The old version of EllipTrack that fully reproduces this tutorial is available in the Google Drive folder. 
