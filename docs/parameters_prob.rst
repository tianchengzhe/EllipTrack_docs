...
*  :greenitalic:`empty_prob` :gray:`(double | advanced parameter)` --- Probability of an event (0 to 1) if no training data is provided.
*  :reditalic:`mitosis_inference_option` :gray:`(string)` --- Method of mitosis inference.
   ...
   Probabilities of "No" events will be modified to 0.5, such that these events' states will not contribute to the probability of cell lineages.
...
*  :reditalic:`migration_option` :gray:`(string)` --- Method of migration probability calculation.
   ...
   For 'similarity', migration probability is calculated by
   ...
   For 'distance', migration probability equals to :math:`P_{mig}\left(D_{i,t}, D_{j,t+1}\right)`.
...
*  :greenitalic:`max_migration_dist_fold` :gray:`(double | advanced parameter)` --- Maximal distance (in folds of the migration speed) a cell can migrate in a frame.
*  :greenitalic:`migration_inference_resolution` :gray:`(double | advanced parameter)` --- 'time' and 'density' only. Resolution of time (in frames) or cell density (in number of cells) for migration speed inference.
*  :greenitalic:`migration_inference_min_samples` :gray:`(integer | advanced parameter)` --- 'time' and 'density' only. Minimal number of samples for migration speed inference.
*  :greenitalic:`prob_nonmigration` :gray:`(double | advanced parameter)` --- Null probability (0 to 1) of migration.
*  :greenitalic:`min_inout_prob` :gray:`(double | advanced parameter)` --- Minimal probability (0 to 1) to migrate in/out of the field of view.
*  :greenitalic:`max_migration_time` :gray:`(integer | advanced parameter)` --- Maximal number of frames in a migration.
